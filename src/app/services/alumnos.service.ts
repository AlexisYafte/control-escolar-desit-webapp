...
export class AlumnosService {
  ...
  public validarAlumnos(data: any, editar: boolean) {
    ...
    let error: any = {};
    //Validaciones
    if (!this.validatorService.required(data['clave_alumnos'])) {
      error['clave_alumnos'] = this.errorService.required;
    }
    ...
    if (!this.validatorService.required(data['curp'])) {
      ...
      alert('La longitud de caracteres deL RFC es menor, deben ser 18');
    } else if (!this.validatorService.max(data['curp'], 18)) {
      error['curp'] = this.errorService.max(18);
      alert('La longitud de caracteres deL RFC es mayor, deben ser 18');
    }
